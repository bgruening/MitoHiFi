...
def filterFasta(
    ...
    if random:
        ...
        filterLengthIdList(
            inStream, outPath, format, minLength, idList, regex, neg, log
        )


def filterLengthIdList(
    inStream,
    outPath,
    format,
    minLength=None,
...
def sampleRandom(inStream, outPath, format, number, log):
    try:
        ...
        if number > l:
            raise ValueError(
                "Sample size(%i) is bigger than number of "
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        dest="regexp",
        default=False,
        help="use regular expression instead of exact " "matching for IDs",
    )
    ...
    parser.add_option(
        ...
        dest="neg",
        default=False,
        help="do exactly the opposite of what would normally " "be done",
    )
    ...
    if len(args) < 1:
        if gzImported and options.gzip:
            parser.error(
                "Pipe mode (no input file argument) does not work together with -z (gzipped input)."
            )
    ...
    if options.idList:
        ...
        try:
            ...
            if log:
                log.write(
                    "ID list parameter is not a valid path. Assume it to "
